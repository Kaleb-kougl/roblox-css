...
describe("webStyle middleware", () => {
	...
	it("individual rounded corners map to UICorner corner radius fields", () => {
		...
		const cornerEl = result.children.find((c: any) => c.props.key === "uicorner");
		expect(cornerEl).toBeDefined();
		expect((cornerEl as any).props.TopLeftRadius.Offset).toBe(10);
		expect((cornerEl as any).props.TopRightRadius.Offset).toBe(20);
		expect((cornerEl as any).props.BottomRightRadius.Offset).toBe(10);
		expect((cornerEl as any).props.BottomLeftRadius.Offset).toBe(30);
	});
	...
	it("individual rounded corners expand shorthand fallbacks", () => {
		...
		const cornerEl = result.children.find((c: any) => c.props.key === "uicorner");
		expect(cornerEl).toBeDefined();
		expect((cornerEl as any).props.TopLeftRadius.Offset).toBe(5);
		expect((cornerEl as any).props.TopRightRadius.Offset).toBe(2);
		expect((cornerEl as any).props.BottomRightRadius.Offset).toBe(3);
		expect((cornerEl as any).props.BottomLeftRadius.Offset).toBe(4);
	});
	...
	it("background gradients can set UIGradient offset and rotation overrides", () => {
		...
		const gradientEl = result.children.find((c: any) => c.props.key === "uigradient");
		expect(gradientEl).toBeDefined();
		expect((gradientEl as any).props.Offset.X).toBe(0.25);
		expect((gradientEl as any).props.Offset.Y).toBe(-0.5);
		expect((gradientEl as any).props.Rotation).toBe(45);
	});
	...
	it("border strokes map native UIStroke controls and alpha", () => {
		...
		const strokeEl = result.children.find((c: any) => c.props.key === "uistroke");
		expect(strokeEl).toBeDefined();
		expect((strokeEl as any).props.Thickness).toBe(4);
		expect((strokeEl as any).props.Transparency).toBe(0.75);
		expect((strokeEl as any).props.BorderOffset.Offset).toBe(2);
		expect((strokeEl as any).props.BorderStrokePosition).toBe(Enum.BorderStrokePosition.Outer);
		expect((strokeEl as any).props.LineJoinMode).toBe(Enum.LineJoinMode.Bevel);
		expect((strokeEl as any).props.StrokeSizingMode).toBe(Enum.StrokeSizingMode.ScaledSize);
		expect((strokeEl as any).props.ZIndex).toBe(3);
	});
	...
	it("strokeTransparency overrides rgba alpha", () => {
		...
		const strokeEl = result.children.find((c: any) => c.props.key === "uistroke");
		expect(strokeEl).toBeDefined();
		expect((strokeEl as any).props.Transparency).toBe(0.2);
	});
	...
	it("strokeGradient injects a child UIGradient on UIStroke", () => {
		...
		const strokeEl = result.children.find((c: any) => c.props.key === "uistroke");
		const gradientEl = (strokeEl as any).props.children;
		expect(strokeEl).toBeDefined();
		expect(gradientEl).toBeDefined();
		expect(gradientEl.props.key).toBe("uistroke-gradient");
		expect(gradientEl.props.Offset.X).toBe(0.1);
	...
	it("textStroke receives native UIStroke controls", () => {
		...
		const strokeEl = result.children.find((c: any) => c.props.key === "uitextstroke");
		expect(strokeEl).toBeDefined();
		expect((strokeEl as any).props.ApplyStrokeMode).toBe(Enum.ApplyStrokeMode.Contextual);
		expect((strokeEl as any).props.LineJoinMode).toBe(Enum.LineJoinMode.Miter);
		expect((strokeEl as any).props.StrokeSizingMode).toBe(Enum.StrokeSizingMode.FixedSize);
		expect((strokeEl as any).props.Transparency).toBe(0.1);
		expect((strokeEl as any).props.ZIndex).toBe(7);
	});
	...
	it("boxShadow can opt into native UIShadow properties", () => {
		...
		const shadowEl = result.children.find((c: any) => c.props.key === "uishadow");
		expect(shadowEl).toBeDefined();
		expect((shadowEl as any).props.Offset.Y.Offset).toBe(6);
		expect((shadowEl as any).props.BlurRadius.Offset).toBe(12);
		expect((shadowEl as any).props.Spread.X.Offset).toBe(2);
		expect((shadowEl as any).props.Transparency).toBe(0.25);
	});
	...
	it("shadow props create native UIShadow without boxShadow", () => {
		...
		const shadowEl = result.children.find((c: any) => c.props.key === "uishadow");
		expect(shadowEl).toBeDefined();
		expect((shadowEl as any).props.BlurRadius.Offset).toBe(8);
		expect((shadowEl as any).props.Offset.X.Offset).toBe(3);
		expect((shadowEl as any).props.Offset.Y.Offset).toBe(-4);
		expect((shadowEl as any).props.Spread.X.Offset).toBe(5);
		expect((shadowEl as any).props.Spread.Y.Offset).toBe(6);
		expect((shadowEl as any).props.Transparency).toBe(0.75);
		expect((shadowEl as any).props.ZIndex).toBe(9);
	});
	...
	it("CSS-like boxShadow can stay on image fallback when requested", () => {
		...
		const imageEl = result.children.find((c: any) => c.props.key === "box-shadow");
		const shadowEl = result.children.find((c: any) => c.props.key === "uishadow");
	...
	it("maps documented list and grid layout ordering controls", () => {
		...
		const listLayout = list.children.find((c: any) => c.props.key === "uilistlayout");
		expect(listLayout).toBeDefined();
		expect((listLayout as any).props.FillDirection).toBe(Enum.FillDirection.Horizontal);
		expect((listLayout as any).props.HorizontalFlex).toBe(Enum.UIFlexAlignment.SpaceEvenly);
		expect((listLayout as any).props.SortOrder).toBe(Enum.SortOrder.Name);
		...
		const gridLayout = grid.children.find((c: any) => c.props.key === "uigridlayout");
		expect(gridLayout).toBeDefined();
		expect((gridLayout as any).props.FillDirectionMaxCells).toBe(3);
		expect((gridLayout as any).props.StartCorner).toBe(Enum.StartCorner.BottomRight);
		expect((gridLayout as any).props.SortOrder).toBe(Enum.SortOrder.Name);
	});
	...
	it("maps documented table and page layout properties", () => {
		...
		const tableLayout = tableResult.children.find((c: any) => c.props.key === "uitablelayout");
		expect(tableLayout).toBeDefined();
		expect((tableLayout as any).props.FillDirection).toBe(Enum.FillDirection.Vertical);
		expect((tableLayout as any).props.MajorAxis).toBe(Enum.TableMajorAxis.ColumnMajor);
		expect((tableLayout as any).props.FillEmptySpaceColumns).toBe(true);
		expect((tableLayout as any).props.FillEmptySpaceRows).toBe(true);
		expect(((tableLayout as any).props.Padding as UDim2).X.Offset).toBe(4);
		expect(((tableLayout as any).props.Padding as UDim2).Y.Offset).toBe(8);
		...
		const pageLayout = page.children.find((c: any) => c.props.key === "uipagelayout");
		expect(pageLayout).toBeDefined();
		expect((pageLayout as any).props.FillDirection).toBe(Enum.FillDirection.Horizontal);
		expect(((pageLayout as any).props.Padding as UDim).Offset).toBe(12);
		expect((pageLayout as any).props.Animated).toBe(true);
		expect((pageLayout as any).props.Circular).toBe(true);
		expect((pageLayout as any).props.TweenTime).toBe(0.35);
		expect((pageLayout as any).props.EasingStyle).toBe(Enum.EasingStyle.Cubic);
		expect((pageLayout as any).props.EasingDirection).toBe(Enum.EasingDirection.InOut);
		expect((pageLayout as any).props.GamepadInputEnabled).toBe(false);
		expect((pageLayout as any).props.ScrollWheelInputEnabled).toBe(false);
		expect((pageLayout as any).props.TouchInputEnabled).toBe(true);
	});
	...
	it("maps documented size, text animation, and CanvasGroup properties", () => {
		...
		const scale = result.children.find((c: any) => c.props.key === "uiscale");
		const textConstraint = result.children.find((c: any) => c.props.key === "uitextsizeconstraint");
		expect(scale).toBeDefined();
		expect((scale as any).props.Scale).toBe(1.5);
		expect(textConstraint).toBeDefined();
		expect((textConstraint as any).props.MinTextSize).toBe(9);
		expect((textConstraint as any).props.MaxTextSize).toBe(48);
